...
#' ---
#' title: "Analiza sentymentu zagranicznych publikacji odnoście wyników wyborów prezydenckich w Polsce."
#' authors: "Barbara Chojnacka, Aleksandra Domańska"
#' date: "06.06.2025"
#' output:
#'    html_document:
#'      df_print: paged
...
plot_wordcloud <- function(freq_df, title, color_palette = "Dark2") {
  # Ograniczenie do top 20 słów
  top_20_freq_df <- head(freq_df, 20)
  wordcloud(words = top_20_freq_df$word, freq = top_20_freq_df$freq, min.freq = 1,
...
# Wyciągnięcie top 20 słów z sentymentem
...
# Filtrowanie tylko pozytywnych i negatywnych emocji
sentiment_review_nrc2 <- sentiment_review_nrc %>%
  filter(sentiment %in% c("positive", "negative"))

# Zliczanie i wybieranie top 20 słów dla każdej kategorii
...
# Filtrowanie tylko pozytywnych i negatywnych słów
sentiment_review_bing2 <- sentiment_review_bing %>%
  filter(sentiment %in% c("positive", "negative"))

# Zliczanie i wybór 20 najczęstszych słów dla każdej kategorii
...
# Wykres
ggplot(word_counts_afinn3, aes(x = word2, y = freq, fill = factor(value))) + 
  geom_col(show.legend = FALSE) +
